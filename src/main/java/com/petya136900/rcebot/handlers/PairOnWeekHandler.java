...
public class PairOnWeekHandler implements HandlerInterface {
	...
	public void handle(VK vkContent) {
		if(vkContent.getVK().getPayload()!=null) {
			...
			try {
				...
				vkContent.getVK().getMessage().setText("пары на "+day);
				vkContent.getVK().getMessage().setPayload(JsonParser.toJson(new Payload().setData("from_keyboard")));
				new TimetableHandler().handle(vkContent);
			} catch (Exception e) {
